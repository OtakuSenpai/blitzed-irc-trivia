...
void Client::get_localhost()
{
  ...
  m_locallongip = (bytes[0] * 256 * 256 * 256) + (bytes[1] * 256 * 256) + (bytes[2] * 256) + (bytes[3]);
}


void Client::connect_to(char *host, int port)
{
   ...
   if( connect(m_sockfd, (struct sockaddr *) &m_server_addr, sizeof(struct sockaddr)) == -1)
    {
...
void Client::s(int priority,const char *data,...)
{
...
void Client::privmsg(char *target, const char *msg, ...)
{
  va_list         arglist;
  char            msg2[1024];
...
void Client::notice(char *target, const char *msg, ...)
{
  va_list         arglist;
  char            msg2[1024];
...
void Client::do_register()
{
   ...
   s(100,"USER %s \"%s\" %s :%s",config->CLIENT_Username,config->CLIENT_Username,config->CLIENT_Username,config->CLIENT_Realname);

}

void Client::get_from_server()
{
   ...
   do
    {
      if(read(m_sockfd,&c,1) < 1)
         return 0;
      if(i >= RECVSIZE) return 0;

      if(c != 13 && c != 10) m_tmpline[i++] = c;
    } while(c != 10);
...
void Client::parse_line(char *line)
{
...
void Client::parse_join(source_struct *source, char *line)
{
...
void Client::parse_kick(source_struct *source, char *line)
{
   strtok(line, " ");
...
void Client::parse_newnick(source_struct *source, char *line)
{
   game->do_newnick(source, line);
}


void Client::parse_part(source_struct *source, char *line)
{
...
void Client::parse_privmsg(source_struct *source, char *line)
{
...
int Client::ignore_check(char *mask)
{
...
void Client::ignore_load()
{
   ...
   if(in.fail())
    {
         log->logtofile("Error opening ignore state file for read. (Might not exist)");
         return;
    }
...
void Client::do_ignore(source_struct *source, char *args)
{
    char *password = 0;
    char *rest     = 0;
...
void Client::ignore_list(source_struct *source)
{
...
void Client::ignore_add(source_struct *source, char *mask)
{
...
void Client::ignore_del(source_struct *source, char *mask)
{
...
void Client::do_ctcp(source_struct *source, char *target, char *msg)
{
   ...
   if(!strcasecmp(ctcp, "VERSION"))
      s(5,"NOTICE %s :\001VERSION %s\001",source->nick,config->CTCP_Version);
...
void Client::alarm()
{
   ...
   if((recvdiff > config->CLIENT_TIME_Timeout) && m_time.last_line.tv_sec != 0)
    {
